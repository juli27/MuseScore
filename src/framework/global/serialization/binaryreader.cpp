...
{
    std::uint8_t b = {};
    Result<std::size_t> bytesRead = readNBytes(&b, 1);
    if (!bytesRead) {
...
BinaryReader::Result<std::vector<std::uint8_t> > BinaryReader::readNBytes(std::size_t n)
{
    ...
    Result<std::size_t> bytesRead = readNBytes(buf.data(), n);
    if (!bytesRead) {
        return Unexpected(std::move(bytesRead).error());
    }
...
BinaryReader::Result<std::size_t> BinaryReader::readNBytes(std::uint8_t* data, std::size_t n)
{
    return readBytes(data, n)
           .and_then([&](const std::size_t bytesRead) -> Result<std::uint64_t> {
        if (bytesRead != n) {
            // revert pos back to what it was
            m_device->seek(m_device->pos() - bytesRead);

            return Unexpected(makeError(ErrCode::TooFewBytes,
                                        String(u"unexpected number of bytes: %1 read, %2 expected")
                                        .arg(bytesRead).arg(n).toStdString()));
        }
...
BinaryReader::Error BinaryReader::makeError(const ErrCode code, std::string message)
{
    return Error{ code, m_device->pos(), std::move(message) };
}
} // namespace muse
